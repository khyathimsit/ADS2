...
public final class In {
     ...
     *
     * @return {@code true} if this input stream exists;
    ...
    /**.
      ...
      * (except possibly whitespace).
      * Use this to know whether the next call to
      * {@link #readString()},
      * {@link #readDouble()}, etc will succeed.
      *
      * @return {@code true} if this input stream is
      * empty (except possibly whitespace);
      *         {@code false} otherwise
      */
    ...
    /**.
      ...
      *
      * @return {@code true} if this input
      * stream has more input (including whitespace);
      *         {@code false} otherwise
      */
     ...
     * the next call to {@link #readChar()} will succeed.
     ...
     * character in this input stream.
     *
    ...
    /**.
      * Reads and returns the remainder of this
    ...
    public String readAll() {
        ...
        String result = scanner.useDelimiter(
            EVERYTHING_PATTERN).next();
        scanner.useDelimiter(WHITESPACE_PATTERN);
        return result;
    }
    /**.
      * Reads the next token from this input
      * stream and returns it as a {@code String}.
      *
    ...
    /**.
      * Reads the next token from this input
      * stream, parses it as a {@code int},
    ...
    /**.
      * Reads the next token from this input
      * stream, parses it as a {@code double},
    ...
    /**
      ...
      * the next token cannot be parsed as a {@code short}
      */
    ...
    /**.
      ...
      * @throws NoSuchElementException if the input stream is empty
      * @throws InputMismatchException if the
      * next token cannot be parsed as a {@code byte}
      */
    ...
    /**.
     * Reads the next token from this input
     * stream, parses it as a {@code boolean}
     * (interpreting either {@code "true"} or
     * {@code "1"} as {@code true},
     * and either {@code "false"} or
     * {@code "0"} as {@code false}).
     *
     * @return the next {@code boolean} in this input stream
     * @throws NoSuchElementException if the input stream is empty
     * @throws InputMismatchException if the
     * next token cannot be parsed as a {@code boolean}
     */
    ...
    /**.
     * Reads all remaining tokens from this
     * input stream and returns them as
     * an array of strings.
     *
     * @return all remaining tokens in this
     * input stream, as an array of strings
     */
    ...
    /**.
     * Reads all remaining lines from
     * this input stream and returns them as
     * an array of strings.
     *
     * @return all remaining lines in
     * this input stream, as an array of strings
     */
     ...
     * {@code new In(filename)}.{@link #readAllInts()}.
     */
    @Deprecated
